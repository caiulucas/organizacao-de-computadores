...
Instruction *generateRandomInstruction(int ramSize)
{
    ...
    instruction->add1.block = rand() % ramSize;
    instruction->add1.word = rand() % WORDS_SIZE;
    instruction->add2.block = rand() % ramSize;
    instruction->add2.word = rand() % WORDS_SIZE;
    instruction->add3.block = rand() % ramSize;
    instruction->add3.word = rand() % WORDS_SIZE;
    instruction->opcode = rand() % 3;

    return instruction;
}

// Mudar para suportar l3
Instruction *readInstructions(char *fileName, int *memoriesSize)
{
    ...
    int n, i = 0;
    fscanf(file, "%d %d %d %d %d %d", &n, &memoriesSize[0], &memoriesSize[1], &memoriesSize[2], &memoriesSize[3], &memoriesSize[4]);
