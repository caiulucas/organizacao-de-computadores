...
Instruction* generateRandomInstructions(int memorySize) {
    ...
    for (int i=0; i<n; i++) {
        instructions[i].add1.block = rand() % memorySize;
        instructions[i].add1.word = rand() % WORDS_SIZE;
        instructions[i].add2.block = rand() % memorySize;
        instructions[i].add2.word = rand() % WORDS_SIZE;
        instructions[i].add3.block = rand() % memorySize;
